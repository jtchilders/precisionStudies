...
def generate_ddabs_test_case():
   """
   Generate a test case for ddabs using a random value.
   """
   # Generate a random value [-1, 1) in high precision
   a = (mp.rand() * 2) - 1

   # Compute expected result
   result = mp.fabs(a)

   # Convert to input and output to double-double format
   ...
   if not (valid_a and valid_result):
      print(
         f"Verification of ddabs failed for tolerance {tolerance}:\n"
...
if __name__ == "__main__":
   print("Generating test cases for ddabs...")
   ntests = 10
   if len(sys.argv) > 1:
