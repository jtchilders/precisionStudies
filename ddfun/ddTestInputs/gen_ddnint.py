...
def generate_ddnint_test_case():
   """
   Generate a test case for ddnint.
   """
   # Generate a random high-precision number within a reasonable range
   a = mp.mpf(mp.rand() * (2 ** 104))

   # Compute expected result using Python's rounding
   result = mp.nint(a)
   ...
   if not (valid_a and valid_result):
      print(
         f"Verification of ddnint failed for tolerance {tolerance}:\n"
...
def generate_test_cases(num_cases):
   return [generate_ddnint_test_case() for _ in range(num_cases)]

def write_test_cases_to_text_file(filename, test_cases):
   """
   Generate test cases for ddnint and write them to a file.
   Each line contains:
...
if __name__ == "__main__":
   print("Generating test cases for ddnint...")
   ntests = 10
   if len(sys.argv) > 1:
      ntests = int(sys.argv[1])
   test_cases = generate_test_cases(ntests)
   write_test_cases_to_text_file("data/ddnint_test_cases.txt", test_cases)
   write_test_cases_to_binary("data/ddnint_test_cases.bin", test_cases)