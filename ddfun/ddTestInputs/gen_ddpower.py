...
def generate_ddpower_test_case():
   """
   Generate a test case for ddpower with valid base a > 0.
   """
   # Generate a random base in range (0,1) and a random exponent [0,1) in high precision
   a = mp.rand() + 0.1  # Ensure a > 0
   b = mp.rand()
   ...
   if not (valid_a and valid_b and valid_result):
      print(
         f"Verification of ddpower failed for tolerance {tolerance}:\n"
...
def generate_test_cases(num_cases):
   return [generate_ddpower_test_case() for _ in range(num_cases)]
...
if __name__ == "__main__":
   print("Generating test cases for ddpower...")
   test_cases = generate_test_cases(10)
   write_test_cases_to_text_file("data/ddpower_test_cases.txt", test_cases)
   write_test_cases_to_binary("data/ddpower_test_cases.bin", test_cases)