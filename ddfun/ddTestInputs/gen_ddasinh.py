...
def generate_ddasinh_test_case():
   """
   Generate a test case for ddasinh.
   """
   # Generate a random value for input A such that it is suitable for asinh
   a = 10 * mp.rand() - 5  # random values between -5 and 5

   # Compute expected result using asinh
   result = asinh(a)
   ...
   if not (valid_a and valid_result):
      print(
         f"Verification of ddasinh failed for tolerance {tolerance}:\n"
...
def generate_test_cases(num_cases):
   return [generate_ddasinh_test_case() for _ in range(num_cases)]

def write_test_cases_to_text_file(filename, test_cases):
   """
   Generate test cases for ddasinh and write them to a file.
   Each line contains:
...
if __name__ == "__main__":
   print("Generating test cases for ddasinh...")
   ntests = 10
   if len(sys.argv) > 1:
      ntests = int(sys.argv[1])
   test_cases = generate_test_cases(ntests)
   write_test_cases_to_text_file("data/ddasinh_test_cases.txt", test_cases)
   write_test_cases_to_binary("data/ddasinh_test_cases.bin", test_cases)