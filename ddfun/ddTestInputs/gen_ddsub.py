...
def generate_ddsub_test_case():
   """
   Generate a test case for ddsub using random values.
   """
   ...
   if not (valid_a and valid_b and valid_result):
      print(
         f"Verification of ddsub failed for tolerance {tolerance}:\n"
...
def generate_test_cases(num_cases):
   return [generate_ddsub_test_case() for _ in range(num_cases)]

def write_test_cases_to_text_file(filename, test_cases):
   """
   Generate test cases for ddsub and write them to a file.
   Each line contains:
...
if __name__ == "__main__":
   print("Generating test cases for ddsub...")
   ntests = 10
   if len(sys.argv) > 1:
      ntests = int(sys.argv[1])
   test_cases = generate_test_cases(ntests)
   write_test_cases_to_text_file("data/ddsub_test_cases.txt", test_cases)
   write_test_cases_to_binary("data/ddsub_test_cases.bin", test_cases)