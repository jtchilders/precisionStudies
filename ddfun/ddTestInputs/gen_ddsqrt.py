...
def generate_ddsqrt_test_case():
   """
   Generate a test case for ddsqrt using random positive numbers as the base value.
   """
   # Generate a random number greater than 0 [1, 2) in high precision to avoid sqrt of zero
   a = mp.rand() + 1

   # Compute expected result (square root)
   result = mp.sqrt(a)
   ...
   if not (valid_a and valid_result):
      print(
         f"Verification of ddsqrt failed for tolerance {tolerance}:\n"
...
if __name__ == "__main__":
   print("Generating test cases for ddsqrt...")
