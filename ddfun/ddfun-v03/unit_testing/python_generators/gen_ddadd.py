...
def write_test_cases_to_file(filename, num_cases=10):
    ...
    with open(filename, "w") as f:
        for _ in range(num_cases):
            test_case = generate_ddadd_test_case()
            f.write(
                f"{test_case['dda'][0]:.16e} {test_case['dda'][1]:.16e} "
                f"{test_case['ddb'][0]:.16e} {test_case['ddb'][1]:.16e} "
...
def generate_binary_test_file(filename, num_cases=10):
    """
    Generate and save test cases for ddadd to a binary file.
    """
    test_cases = [generate_ddadd_test_case() for _ in range(num_cases)]
    write_test_cases_to_binary(filename, test_cases)
    print(f"Test cases successfully written to {filename}")

if __name__ == "__main__":
    write_test_cases_to_file("ddadd_test_cases.txt", num_cases=10)
    generate_binary_test_file("ddadd_test_cases.bin", num_cases=10)
