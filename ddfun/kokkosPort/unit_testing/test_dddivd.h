#ifndef TEST_DDDIVD_H
#define TEST_DDDIVD_H
...
// Kokkos-based unittest for dddivd
void unittest_dddivd(const std::string &filename) {
   ...
   std::cout << "Running tests for dddivd from: " << filename << std::endl;
   ...
   Kokkos::View<ddouble *> da("da", total_tests);
   Kokkos::View<double *> db("db", total_tests);
   auto ha = Kokkos::create_mirror_view(da);
   auto hb = Kokkos::create_mirror_view(db);

   Kokkos::View<ddouble *> dexpected("dexpected", total_tests);
   ...
   // Perform dddivd in parallel
   Kokkos::parallel_for("test_dddivd", total_tests, KOKKOS_LAMBDA(const int i) {
      dresult(i) = dddivd(da(i), db(i));
   });
