...
// Kokkos-based unittest for dddiv
void unittest_dddiv(const std::string &filename) {
   ...
   std::cout << "Running tests for dddiv from: " << filename << std::endl;
   ...
   // Perform dddiv in parallel
   Kokkos::parallel_for("test_dddiv", total_tests, KOKKOS_LAMBDA(const int i) {
      result(i) = a(i) / b(i);
   });
