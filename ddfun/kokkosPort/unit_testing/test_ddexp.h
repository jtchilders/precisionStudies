...
// Kokkos-based unittest for ddexp
void unittest_ddexp(const std::string &filename) {
   ...
   // Perform ddexp in parallel
   Kokkos::parallel_for("test_ddexp", total_tests, KOKKOS_LAMBDA(const int i) {
      result(i) = ddexp(a(i));
   });
   ...
   for (int i = 0; i < total_tests; i++) {
      ...
      if (std::isinf(val_result.hi) && val_result.lo == 0.0) {
         if (std::isinf(val_expected.hi) && val_expected.lo == 0.0) {
            passed_tests++;
            continue;
         } else {
            std::cout << "Test Failed: Overflow with input: " << val_a
                      << " expected: [Inf, 0.0]\n";
            continue;
         }
      }
